...
public:
    ...
    void upload_griglia(griglia input_griglia){

        int larg_inp_griglia = input_griglia.get_larg();
        int alt_inp_griglia = input_griglia.get_alt();

        int x_inp_griglia = input_griglia.get_x();
        int y_inp_griglia = input_griglia.get_y();

        int riga_griglia;
        int colonna_griglia;

        for(int k = 0; k < alt_inp_griglia; k++){
            for(int j = 0; j < larg_inp_griglia; j++){

                riga_griglia = y_inp_griglia - pos_y + k;
                colonna_griglia = x_inp_griglia - pos_x + j;

                update_griglia(riga_griglia, colonna_griglia, input_griglia.get_griglia_elem(k, j));
    ...
    void remove_row(int ind_riga){
        ...
        if(ind_riga == 0){
            for(int j = 1; j <= larg - 2; j++){update_griglia(0, j, 32);}
        }
